...
const { KannasoStatus } = require('../constants/KannasoConstants.js');
const { CONNECTED, CONNECTING, DISCONNECTING, DISCONNECTED } = KannasoStatus;
const KannasoError = require('../constants/KannasoError.js');
const { wait } = require('../util/KannasoUtil.js');

/**
 * KannasoLink, contains data about the voice connection on the guild.
 * @class KannasoLink
 * @extends {EventEmitter}
 */
class KannasoLink extends EventEmitter {
    /**
     * @param {KannasoPlayer} player The player of this link.
     * @param {KannasoSocket} node The node that governs this link.
    ...
    constructor(player, node, guild) {
        ...
        /**
         * If this link is reconnecting via KannasoLink.attemptReconnect() or KannasoLink.moveToNode()
         * @type {boolean}
         */
    ...
    */
    get shardPing() {
        return this.shard ? this.shard.ping : this.node.Kannaso.client.ws.ping;
    }

    get guild() {
        return this.node.Kannaso.client.guilds.cache.get(this.guildID);
    }

    get shard() {
        return this.node.Kannaso.client.ws.shards.get(this.shardID);
    }

    /**
     * Attempts to reconnect this KannasoLink, A use case for this is when your Discord Websocket re-identifies
     * @memberOf KannasoLink
    ...
    async attemptReconnect({ voiceChannelID, forceReconnect } = {}) {
        if (this.state === DISCONNECTED || !this.serverUpdate || forceReconnect) {
            if (!voiceChannelID)
                throw new KannasoError('Please specify the channel you want this node to connect on');
            try {
    ...
    async moveToNode(node) {
        try {
            if (!node) throw new KannasoError('No available nodes to reconnect to');
            if (!(node instanceof this.node.constructor)) throw new KannasoError('Node is not an instance of KannasoSocket');
    ...
    setStateUpdate({ session_id, channel_id, self_deaf, self_mute }) {
        ...
        if (!session_id) return this.authenticateFailed(new KannasoError('No session_id intact on Discord State Update OP'));
        this.sessionID = session_id;
        this.node.emit('debug', this.node.name,
    ...
    connect(options) {
        return new Promise((resolve, reject) => {
            if (!options)
                return reject(new KannasoError('No options supplied'));
            if (this.state === CONNECTING)
                return reject(new KannasoError('Can\'t connect while a connection is connecting. Wait for it to resolve first'));
            ...
            this.connectTimeout = setTimeout(() => {
                this.authenticateFailed(new KannasoError('The voice connection is not established in 15 seconds'));
                this.node.emit('debug', this.node.name,
